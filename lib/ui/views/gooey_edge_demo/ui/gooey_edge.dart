...
class GooeyEdge {
  ...
  GooeyEdge({int count = 10, this.side = Side.left}) {
    ...
    for (int i = 0; i < count; i++) {
      points.add(GooeyPoint(x: 0.0, y: i / (count - 1)));
    }
  }
  ...
  Path? buildPath(Size size, {double margin = 0.0}) {
    ...
    Path path = Path();
    int l = points.length;
    Offset pt = GooeyPoint(x: -margin, y: 1.0).toOffset(transform: mtx), pt1;
    path.moveTo(pt.dx, pt.dy); // bl

    pt = GooeyPoint(x: -margin, y: 0.0).toOffset(transform: mtx);
    path.lineTo(pt.dx, pt.dy); // tl

    pt = points[0].toOffset(transform: mtx);
    path.lineTo(pt.dx, pt.dy); // tr

    pt1 = points[1].toOffset(transform: mtx);
    path.lineTo(pt.dx + (pt1.dx - pt.dx) / 2, pt.dy + (pt1.dy - pt.dy) / 2);

    for (int i = 2; i < l; i++) {
      pt = pt1;
      pt1 = points[i].toOffset(transform: mtx);
  ...
  void _addPointTension(GooeyPoint pt0, double x, double t) {
...
class GooeyPoint {
  ...
  GooeyPoint({this.x = 0.0, this.y = 0.0});
